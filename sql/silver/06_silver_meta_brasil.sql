DROP TABLE IF EXISTS {{ silver_database }}.meta_alfabetizacao_brasil;

CREATE TABLE {{ silver_database }}.meta_alfabetizacao_brasil
WITH (
    format = 'PARQUET',
    external_location = 's3://{{ bucket }}/{{ environment }}/{{ silver_path }}/meta_alfabetizacao_brasil/',
...
WITH bronze_padronizada AS (
    ...
    FROM {{ bronze_database }}.meta_alfabetizacao_brasil
),
