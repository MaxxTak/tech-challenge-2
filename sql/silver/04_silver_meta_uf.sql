DROP TABLE IF EXISTS {{ silver_database }}.meta_alfabetizacao_uf;

CREATE TABLE {{ silver_database }}.meta_alfabetizacao_uf
WITH (
    format = 'PARQUET',
    external_location = 's3://{{ bucket }}/{{ environment }}/{{ silver_path }}/meta_alfabetizacao_uf/',
...
WITH bronze_padronizada AS (
    ...
    FROM {{ bronze_database }}.meta_alfabetizacao_uf
),
...
bronze_com_flags AS (
    SELECT
        *,
        ...
        CASE
            WHEN
                taxa_alfabetizacao_pct IS NULL
                AND meta_alfabetizacao_2024_pct IS NULL
                AND meta_alfabetizacao_2025_pct IS NULL
                AND meta_alfabetizacao_2026_pct IS NULL
                AND meta_alfabetizacao_2027_pct IS NULL
                AND meta_alfabetizacao_2028_pct IS NULL
                AND meta_alfabetizacao_2029_pct IS NULL
                AND meta_alfabetizacao_2030_pct IS NULL
                AND percentual_participacao_pct IS NULL
            THEN false
            ELSE true
        END AS flag_registro_com_indicadores,
